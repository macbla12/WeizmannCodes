void ReproducePlotCMSAnal()
{

    
    TString File ="Files/dataOOwithTracks.root"; 
    double Centrality[]={241.358, 204.217,176.384,153.033, 132.738,  114.885, 99.082, 85.128, 72.841, 62.061, 52.594,  44.348, 37.123, 30.774};
    //double Nparticipates[]={26.49, 24.04, 21.80, 19.52, 17.29, 15.22, 13.34, 11.64, 10.11, 8.75, 7.54, 6.47, 5.52, 4.72}; //ATLAS
    double Nparticipates[]={26.3, 24.0, 22.0, 20.1, 18.1, 16.1, 14.1, 12.3, 10.7, 9.2, 7.9, 6.8, 5.9, 5.1}; //CMS

    double Extrapolation[]={1.364, 1.361, 1.379, 1.375, 1.400, 1.378, 1.381, 1.400, 1.398, 1.388, 1.403, 1.469, 1.536, 1.572};
    //double Extrapolation[]={1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364, 1.364};
    ...
    TFile *Output = new TFile("Plots/ReproducedPlotCMS.root","RECREATE");
    ...
    int eventID=0;
    double NumberOfTracksBins[14] = {0.0};
    double NumberOfEventsBins[14] = {0.0};
    ...
    double FinalNumberOfTracksBin[14]={0.0};
    for(int bin=0; bin<14;bin++)
    {
        FinalNumberOfTracksBin[bin]= NumberOfTracksBins[bin]*(Extrapolation[bin])/(NumberOfEventsBins[bin]*Nparticipates[bin]);
        cout<<bin+1<<" Bin: N particles "<<Nparticipates[bin]<<" 1/<Npart>dn/deta="<<FinalNumberOfTracksBin[bin]<<endl;


    }

    cout << "0-5%: Npart=" <<Nparticipates[0]<< " 1/<Npart>dn/deta=" << FinalNumberOfTracksBin[0] <<"  dn/deta="<<FinalNumberOfTracksBin[0]*Nparticipates[0]<<endl;
    ...
    TGraph *graph = new TGraph(14, Nparticipates, FinalNumberOfTracksBin);
